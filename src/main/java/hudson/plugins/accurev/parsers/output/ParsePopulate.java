...
/**
 * Filters the output of the populate command and just shows a summary of the output. Helps prevent
 * build logs being clogged up with the checkout.
 */
public final class ParsePopulate implements ICmdOutputParser<Boolean, OutputStream> {
  ...
  public Boolean parse(InputStream cmdOutput, OutputStream streamToCopyOutputTo)
    ...
    final Writer stringWriter =
        new OutputStreamWriter(streamToCopyOutputTo, Charset.defaultCharset());
    ...
    try (BufferedReader reader = new BufferedReader(stringReader)) {
      ...
      final String msg =
          "Populated " + countOfElements + " elements and " + countOfDirectories + " directories.";
